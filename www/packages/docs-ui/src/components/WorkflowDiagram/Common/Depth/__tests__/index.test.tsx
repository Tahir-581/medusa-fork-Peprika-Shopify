...
describe("rendering", () => {
  test("renders step nodes for cluster", () => {
    ...
    const { container } = render(
      <WorkflowDiagramDepth cluster={cluster} next={next} />
    )
    const nodes = container.querySelectorAll("[data-testid='step-node']")
    expect(nodes).toHaveLength(2)
    expect(nodes[0]).toHaveAttribute("data-step-name", "test.step1")
    expect(nodes[1]).toHaveAttribute("data-step-name", "test.step2")
  })
